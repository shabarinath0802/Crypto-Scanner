"""
Crypto 1H EMA55 Breakdown Scanner
------------------------------------
Scans every USDT pair on Binance. On the most recently CLOSED 1-hour
...
Runs hourly via GitHub Actions -- no persistent memory needed, since
a fresh cross only happens once per transition and the hourly cadence
naturally prevents repeat alerts for the same cross.
"""
...
def send_email(symbols):
    ...
    body = "EMA55 just crossed BELOW EMA8, EMA13, and EMA21 on the 1H candle for:\n\n" + "\n".join(symbols)

    msg = MIMEText(body)
    msg["Subject"] = f"EMA55 Breakdown Alert ({len(symbols)} coins)"
    msg["From"] = sender
    msg["To"] = recipient
...
def main():
    ...
    if hits:
        ...
        print("No email sent (no breakdowns this hour).")
